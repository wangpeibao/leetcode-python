...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:  # hash
        has_dict = dict()
        for dpdomain in cpdomains:
            count, dpstr = dpdomain.split(" ")
            dp_list = dpstr.split(".")
            for index in range(len(dp_list)):
                key = ".".join(dp_list[index:])
                if key in has_dict.keys():
                    has_dict[key] += int(count)
                else:
                    has_dict[key] = int(count)
        result = []
