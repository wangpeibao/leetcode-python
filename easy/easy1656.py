    ...
    def insert(self, id: int, value: str) -> List[str]:
        ...
        if id == self.ptr:
            while self.ptr <= len(self.data):
                if self.data[self.ptr - 1]:
                    result.append(self.data[self.ptr - 1])
                    self.ptr += 1
                else:
                    break
        return result
...
so = OrderedStream(5)
print(so.insert(3, "ccccc"))
print(so.insert(1, "aaaaa"))
print(so.insert(2, "bbbbb"))
print(so.insert(5, "eeeee"))
print(so.insert(4, "ddddd"))
