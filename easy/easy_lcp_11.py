...
小 A 和小 B 负责审核面试者，他们均有所有面试者的简历，并且将各自根据面试者能力值从大到小的顺序浏览。由于简历事先被打乱过，
能力值相同的简历的出现顺序是从它们的全排列中等可能地取一个。现在给定 n 名面试者的能力值 scores，
...
so = Solution()
print(so.expectNumber(scores = [1,2,3]) == 3)
print(so.expectNumber(scores = [1,1]) == 1)
print(so.expectNumber(scores = [1,1,2]) == 2)